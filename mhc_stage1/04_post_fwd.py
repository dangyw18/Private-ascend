...
def gpu_mhc_post_fwd(mhc: int, hidden: int, n_thr: int = 128, h_blk: int = 1024) -> tilelang.JITKernel:
    n = T.dynamic('num_tokens')
    h = hidden
    ...
    return gpu_mhc_post_fwd_kernel
...
if __name__ == "__main__":
    ...
    gpu_program = gpu_mhc_post_fwd(mhc, hidden)
    gpu_kernel = tilelang.compile(gpu_program, target="ascend", out_idx=-1)
    # gpu_actual = kernel(a, b, c, d)
    # expected = ref_program(a, b, c, d)
    # torch.npu.synchronize()
    # torch.testing.assert_close(gpu_actual, expected, rtol=1e-2, atol=1e-2)
    # print("PASS: mhc_post_fwd")