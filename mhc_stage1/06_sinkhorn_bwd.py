...
if __name__ == "__main__":
    ...
    token_block_size = 32
    repeat, eps = 2, 1e-6
    grad_output = torch.randn((num_tokens, hidden_size, hidden_size), device="npu", dtype=torch.float32)
    x = torch.randn((num_tokens, hidden_size, hidden_size), device="npu", dtype=torch.float32)

    program = _mhc_sinkhorn_bwd(hidden_size, token_block_size, repeat, eps)
    kernel = tilelang.compile(program, target="ascend", out_idx=-1)
    grad_input = kernel(grad_output, x)
    torch.npu.synchronize()
    expected = ref_program(grad_output, x, repeat, eps)
